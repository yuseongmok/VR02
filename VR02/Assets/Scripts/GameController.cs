...
public class GameController : MonoBehaviour
{
    ...
    void SendRayCast()     //마우스 클릭시 Ray 발사
    {
        ...
        if (Physics.Raycast(ray, out hit))                                          //hit 된 것이 있을 경우
        {
            ...
            if(slot.state == Slot.SLOTSTATE.FULL && carryingItem == null)           //선택된 슬롯에서 아이템을 잡고 이동하는 경우
            {
                ...
                carryingItem = itemGO.GetComponent<ItemInfo>();                                    //잡은 아이템을 carringItem에 입력
            ...
            {
                slot.CreateItem(carryingItem.itemld);                               //잡고 있던 아이템 아이디를 가져와서 슬롯에 생성하고
    ...
    void OnItemSelected()
    {
        _target = Camera.main.ScreenToViewportPoint(Input.mousePosition);     //월드 좌표에서 마우스 포지션 값을 가져와서 _target 입력
        _target.z = 0;                                                        //2D
    ...
    {
        var slot = GetSlotById(targetSlotld);                 //기존 슬롯에 있는 오브젝트를 가져와서 파괴
    ...
    void PlaceRandomItem()                          //랜덤한 슬롯에 아이템 배치
    {
        ...
        var rand = UnityEngine.Random.Range(0, slots.Length);                      //슬롯 갯수에 따라서 랜덤 번호를 rand 에 입력
        var slot = GetSlotById(rand);                                              //Rand 받은 index값을 통해서 slot 객체를 가져온다.

        while (slot.state == Slot.SLOTSTATE.FULL)                                  //슬롯 상태가 FULL일 때 까지 랜덤 번호를 찾아서 진행. 
        {
    ...
    bool AllslotsOccupied()                 //슬롯이 채워져 있는지 확인 하는 변수
    {
        foreach(var slot in slots)          //슬롯 배열을 하나씩 환인하면서 foreach문 반복
        {
